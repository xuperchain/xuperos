...
func NewRpcServMG(scfg *sconf.ServConf, engine engines.BCEngine) (*RpcServMG, error) {
	...
	obj := &RpcServMG{
		...
		engine:   xosEngine,
		log:      log,
		rpcServ:  NewRpcServ(engine.(ecom.Engine), scfg, log),
		isInit:   true,
		exitOnce: &sync.Once{},
	}
...
func (t *RpcServMG) Run() error {
	...
	err := t.runRpcServ()
	if err != nil {
		t.log.Error("endorser grpc server abnormal exit", "err", err)
		return err
	}

	t.log.Trace("endorser grpc server exit")
	return nil
}
...
func (t *RpcServMG) runRpcServ() error {
	...
	pb.RegisterXendorserServer(t.servHD, t.rpcServ)

	lis, err := net.Listen("tcp", fmt.Sprintf(":%d", t.scfg.EndorserRpcPort))
	if err != nil {
